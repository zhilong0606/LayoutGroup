...
public class UIScrollView : UIBehaviour, IInitializePotentialDragHandler, IBeginDragHandler, IEndDragHandler, IDragHandler, IScrollHandler, ICanvasElement, ILayoutElement, ILayoutGroup
{
        ...
        if (!m_Dragging && (offset != Vector2.zero || m_Velocity != Vector2.zero))
        {
            ...
            for (int axis = 0; axis < 2; axis++)
            {
                ...
                if (m_MovementType == MovementType.Elastic && offset[axis] != 0)
                {
                    ...
                    //speed += Mathf.Sign(speed) * -1 * a * deltaTime;
                    //if (speed * m_Velocity[axis] <= 0f)
                    //{
                    //    speed = 0f;
                    //    Debug.LogError("Stop");
                    //}
                    //position[axis] = speed * deltaTime;
                    position[axis] = Mathf.SmoothDamp(m_Content.anchoredPosition[axis], m_Content.anchoredPosition[axis] + offset[axis], ref speed, m_Elasticity, Mathf.Infinity, deltaTime);
                    if (Math.Abs(position[axis] - m_Content.anchoredPosition[axis]) < 1f)
                    {
                        m_Velocity[axis] = 0f;
                    }
                    else
                ...
                else if (m_Inertia)
                {
                    ...
                    if (m_Velocity[axis] > 0 && Mathf.Abs(m_Velocity[axis]) < 1)
                    {
                        m_Velocity[axis] = 0;
                        Debug.LogError("StopInertia");
                    }
